...
def score_dataset(dataset, models, batch_size):
    ...
    accelerator.print("Scoring dataset...")
    ...
    for group_id in range(n_groups):
        ...
        accelerator.print(f"Scoring with model for group {group_id}...")
        ...
        #model, dataloader = accelerator.clear(model, dataloader)
...
def assign_group_ids_fast(dataset, n_groups, num_users):
    """
...
def assign_group_ids(dataset, n_groups, num_users):
...
def clustering(dataset, models, batch_size, num_users):
    dataset = score_dataset(dataset, models, batch_size)
    dataset = assign_group_ids_fast(dataset, len(models), num_users)
    #dataset = assign_group_ids(dataset, len(models), num_users)
    ...
    dataset = dataset.remove_columns([f"score_r{i}_group{g}" for i in range(2) for g in range(len(models))])
    return dataset

def update_reward_model(model, dataset, tokenizer, seed, training_args):
