...
def compute_scores(example):
    if example["group_name"] == "minority":
        score0 = example["helpful_score_0"] 
        score1 = example["helpful_score_1"]

    if example["group_name"] == "majority":
        score0 = 0.3*example["helpful_score_0"] + 0.7*example["harmless_score_0"]
        score1 = 0.3*example["helpful_score_1"] + 0.7*example["harmless_score_1"]
...
def create_chosen_rejected_dataset(
    ...
    dataset = dataset.remove_columns(["prompt", "group_name", "response_0", "response_1", "helpful_score_0", "helpful_score_1", "harmless_score_0", "harmless_score_1", "score_0", "score_1"])
    return dataset
...
def main():
    ...
    # Load the dataset
    dataset = load_dataset(f"{user_id}/tinyllama-hh-raw-10K-test", split="train")
    dataset = assign_group_id(dataset, seed, proportion)
    ...
    dataset.push_to_hub(
        f"{user_id}/tinyllama-hh-10K-seed{seed}-prop{proportion}",
    )
